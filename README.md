# Crazyflie TinyMPC API  [![CI](https://github.com/bitcraze/crazyflie-firmware/workflows/CI/badge.svg)](https://github.com/bitcraze/crazyflie-firmware/actions?query=workflow%3ACI)

This project contains the OOT Controller API used for TinyMPC
...
This repository uses git submodules. Clone with the `--recursive` flag, the `GIT_CLONE_PROTECTION_ACTIVE=false` flag, and to have `git-lfg` installed.

```
$ GIT_CLONE_PROTECTION_ACTIVE=false git clone git@github.com:ginoekzhang/a2r-tinympc-crazyflie-firmware.git --recursive
```
